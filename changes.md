...
* グローバル変数 `stt_zero@` はやめて、`defvar@__abdata` を用いるようになった。
* 各モジュールの `#include` がちょうど1回だけ行われるようにした。
  * `all.hsp` が適切な順番で `#include` を行う。
  * インクルードガードがあるので重複があっても問題なかったが、プリプロセッサに負荷がかかる。
  * 外部に公開されるヘッダが `all.hsp` と `all.as` だけになった。
